...
def cover_letter_template_list_create(request):
    if request.method == "GET":
        templates = CoverLetterTemplate.objects.all()  # no is_active filter
        serializer = CoverLetterTemplateSerializer(templates, many=True)
...
def cover_letter_template_detail(request, pk):
    template = get_object_or_404(CoverLetterTemplate, pk=pk)
    ...
    if request.method in ["PUT", "PATCH"]:
        ...
        serializer = CoverLetterTemplateSerializer(template, data=request.data, partial=(request.method == "PATCH"))
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
