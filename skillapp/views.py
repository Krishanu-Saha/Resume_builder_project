...
def skill_list_create(request, resume_id):
    """
    List all skills for a resume (owned by the user) or create a new skill.
    """
...
def skill_detail(request, resume_id, pk):
    skill = get_object_or_404(Skill, pk=pk, resume_id=resume_id)
    
    # Ownership check
    if skill.resume.user != request.user:
    ...
    if request.method in ['PUT', 'PATCH']:
        serializer = SkillSerializer(skill, data=request.data, partial=(request.method == 'PATCH'), context={'request': request})
        if serializer.is_valid():
            serializer.save(resume=skill.resume)  # prevent changing resume
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
